...
export default function Bill() {
  ...
  return (
    <div className="border border-[#FDEACE] p-4 mt-6 rounded-lg flex flex-col gap-2 sticky top-24 bg-[#FFF9F1]">
      <div>
        <Span size={24} color="Primary/950">
          ยอดรวมตะกร้าสินค้า
        </Span>
      </div>
      ...
      <div className="flex justify-between g-3">
        <Span size={16} color="Primary/950" width="SemiBold">
          ยอดรวมตะกร้าสินค้า
        </Span>
        <Span size={16} color="Primary/950" width="SemiBold">{`฿ ${formatCurrency(
          sumWithInitial?.price.toFixed(2)
        )}`}</Span>
      </div>
    </div>
  );
